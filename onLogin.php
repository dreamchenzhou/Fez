<?php
    ...
    //获取盐值
    if($account && $password){
        $sql_sel_user = "select salt from SM_USER where status = 1 and account=$account limit 1";
            //echo $sql_sel_user;
    ...
    if($conn && $errCode != 0){
        ...
        if(mysqli_select_db($conn, $dbname)){
            $sql_sel_user = "select userId,telephone,account,password,salt,role,openId,nickName,avatarUrl,gender,city,province,country,language,status,createdBy,createdAt,updatedBy,updatedAt from SM_USER where status = 1 and (account='$user->account' or openId='$user->openId') limit 1";
            //echo $sql_sel_user;
            $result = mysqli_query($conn, $sql_sel_user);
            if(mysqli_num_rows($result)>0){ //如果数据库中存在此用户信息，则不需要重新获取
                ...
                if($row=mysqli_fetch_assoc($result)){
                    //echo '输出数据';
                //{   
                    if($user->account == $row['account'] && $user->password == $row['password']){  //判断账号是否存在
                        ...
                        $data = $user;
                    } else if($user->openId == $row['openId']){ //判断openId是否存在，直接用微信登录
                        $user->userId = $row['userId'];
                        $user->isLogin = 1;
                        $user->isAuth = 1;
                        //echo '账号密码匹配';
